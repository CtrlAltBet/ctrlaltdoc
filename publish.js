...
// Test RUN
//❯ node ./node_modules/jsdoc/jsdoc ./ -t ./assets/ctrlaltdoc
...
function toClassName(name) {
  // remove <, >, # from type names for display purposes
  // remove html tags as well, since they can cause problems in CSS class names
  return (
    name
      .replace(/<\/?[^>]+(>|$)/g, " ")
      //   &lt;, &gt;, <, >, # are all removed because they can cause problems in CSS class names
      .replace(/&lt;|&gt;|<|>|#/g, " ")
      .replace(/\./g, " ")
      //Remove extra spaces
      .replace(/\s+/g, " ")
...
function typeEncapsulate(text) {
  if (Array.isArray(text)) {
    return text.map(function (t) {
      return typeEncapsulate(t);
    });
  } else {
    //   Contains dot but no html tag
    if (text.indexOf(".") > -1 && !/<.*>/.test(text)) {
      var parts = text
        .split(".")
        .map((part) => `<span class="type-part ${toClassName(part)}">${part}</span>`)
        .join(".");
      return parts;
    } else {
      return `<span class="type-signature ${toClassName(text)}">${text}</span>`;
    }
  }
}

function signatureEncapsulate(text) {
  // (receivedHash, sid, userId, username, trx, privateKey)
  // to=> <span>param1</span>, <span>param2</span>, ...
  const blocks = text
    .replace(/\(|\)/g, " ")
    .split(",")
    .map(
      (param) =>
        `<span class="signature-param param-${param.trim()}">${param.trim()}</span>`,
    )
    .join(", ");
  return `(${blocks})`;
}
...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
  ...
  if (items && items.length) {
    ...
    var level =
      typeof ctrlaltdoc.navLevel === "number" && ctrlaltdoc.navLevel >= 0
        ? ctrlaltdoc.navLevel
        : Infinity;
      ...
      if (!hasOwnProp.call(item, "longname")) {
        ...
        if (methods.length) {
          ...
          methods.forEach(function (method) {
            ...
            if (ctrlaltdoc.private === false && method.access === "private") return;
...
function buildNav(members, { only = "", exclude = [] } = {}) {
  ...
  var ctrlaltdoc = (env && env.conf && env.conf.ctrlaltdoc) || {};
  if (ctrlaltdoc.menu) {
    for (var menu in ctrlaltdoc.menu) {
      nav += "<h2><a ";
      //add attributes
      for (var attr in ctrlaltdoc.menu[menu]) {
        nav += attr + '="' + ctrlaltdoc.menu[menu][attr] + '" ';
      }
      nav += ">" + menu + "</a></h2>";
    }
  }
...
exports.publish = function (taffyData, opts, tutorials) {
  ...
  ctrlaltdoc.sort !== false && data.sort("longname, version, since");
  helper.addEventListeners(data);

  var sourceFiles = {};
  var sourceFilePaths = [];
  data().each(function (doclet) {
    if (ctrlaltdoc.removeQuotes) {
      if (ctrlaltdoc.removeQuotes === "all") {
        if (doclet.name) {
      ...
      } else if (ctrlaltdoc.removeQuotes === "trim") {
        if (doclet.name) {
  ...
  if (packageInfo) {
    ...
    if (packageInfo.name) {
      ...
      if (packageName.length > 1 && ctrlaltdoc.scopeInOutputPath !== false) {
        subdirs.push(packageName[0]);
      }

      if (ctrlaltdoc.nameInOutputPath !== false) {
        subdirs.push(packageName.length > 1 ? packageName[1] : packageName[0]);
      }
    }

    if (packageInfo.version && ctrlaltdoc.versionInOutputPath !== false) {
